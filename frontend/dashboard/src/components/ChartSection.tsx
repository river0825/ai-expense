...
export default function ChartSection({ metrics, aiCosts, currency = 'USD' }: ChartSectionProps) {
            ...
            <ResponsiveContainer width="100%" height={300}>
              <BarChart data={expensesData}>
                ...
                <Tooltip
                  contentStyle={{ backgroundColor: '#1e293b', border: '1px solid #475569' }}
                  labelStyle={{ color: '#e2e8f0' }}
                  formatter={(value: number) => [new Intl.NumberFormat('en-US', { style: 'currency', currency }).format(value), 'Total Expenses']}
                />
                <Legend />
                <Bar dataKey="total" fill="#10b981" name={`Total Expenses (${currency})`} />
